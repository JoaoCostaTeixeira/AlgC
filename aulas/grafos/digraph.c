...
int VertexType (PtDigraph pdig, unsigned int pv)
{
	  ...
	  PtBiNode Vert;
		if((Vert = OutPosition(pdig->Head,pv))==NULL){
			return NO_VERTEX;
		}
...
int VertexOutDegreeCentrality (PtDigraph pdig, unsigned int pv, double *pcent)
{
	...
	PtVertex Vertex = (PtVertex) Vert->PtElem;
	*pcent =  ((double)(Vertex->OutDeg) / (double)(pdig->NVertexes-1));
	return OK;
}
...
int MaxOutDegreeCentrality (PtDigraph pdig, unsigned int *pv, double *pmax)
{
	...
	PtBiNode n = pdig -> Head ;
	double a = 0;
	for(int i=0; i<pdig -> NVertexes ; i++)
	{
			VertexOutDegreeCentrality(pdig, n -> Number, &a);
			if(*pmax<a){
				*pv=n-> Number;
				*pmax = a;
			}
...
int AverageSucessorOutDegree (PtDigraph pdig, unsigned int pv, double *pmsuc)
{
	...
	if((search = OutPosition(pdig->Head,pv))==NULL){
			return NO_VERTEX;
		}
	...
	double elems=0;
	unsigned int val=0;
	if(VertexType(pdig,pv) == SOURCE || VertexType(pdig,pv) == OK){
		search = search->PtAdj;
		while(search != NULL){
			if((vertex =( (PtVertex)(OutPosition(pdig->Head,search -> Number)->PtElem))) != NULL){
				val += (vertex->OutDeg);
				elems++;
				search = search->PtNext;
...
int AllIsolates (PtDigraph pdig, PtQueue *pqueue)
{
	...
	*pqueue = QueueCreate(sizeof(int));
	for(int i=0; i<pdig -> NVertexes ; i++)
	{
			if(VertexType(pdig,n->Number)==DISC){
				QueueEnqueue(*pqueue, &n->Number);
			}
			n = n -> PtNext;
		}
	return OK;
	...
	while(pHead)
	{
	    if (pHead->Number != pv)
        {
            ...
            while (pAux)
            {
               if (pAux->Number == pv)
							 {
                    QueueEnqueue(*pqueue, &pHead->Number);
										break;
								}else{pAux = pAux->PtNext;}
            }
          }
        pHead = pHead->PtNext;
    }
...
int AllNonEdges (PtDigraph pdig, PtQueue *pqueue)
{
	...
	*pqueue = QueueCreate(sizeof(int));
	while(pHead)
		{
				for(int i = 1; i <= pdig->NVertexes ; i++)
				{
						if (pHead->Number != i)
						{
							pAdj = pHead->PtAdj;

...
PtABPNode ABPCreate (void)
{
  ...
  if ((proot = (PtABPNode) malloc (sizeof (struct abpnode))) == NULL)
...
PtABPNode ABPCeilValue (PtABPNode proot, int pvalue)
{
	...
	if (proot-> Elem == pvalue)
  {
	...
	if (proot-> Elem >  pvalue)
  {
		left = ABPCeilValue(proot->PtLeft, pvalue);
		if (left == NULL)
    {
			Error = OK;
			return proot;
	...
	right = ABPMaxRec(proot);

	if(right-> Elem < pvalue)
  {
		Error = NO_ELEM;
		return NULL;
	}
...
int ABPRank (PtABPNode proot, int pvalue)
{
   ...
   {
      return count + ABPRank(proot->PtRight,pvalue);
   }else if(proot ->PtRight == NULL)
   {
      return count + ABPRank(proot->PtLeft,pvalue);
   }
   return count + ABPRank(proot->PtLeft,pvalue) + ABPRank(proot->PtRight,pvalue);
  }
...
{ 	// NULL_PTR, NO_MEM
    ...
    ABPElements(proot -> PtRight, pqueue, plow, phigh);
  	...
  	if(QueueIsEmpty(pqueue)){ Error = INVALID; return;}

    int sizeque = 0;
    int aux,aux2;
    PtQueue pqueue2 = QueueCreate (sizeof(int));
    while(!QueueIsEmpty(pqueue))
    {
      QueueDequeue(pqueue,&aux);
      QueueEnqueue(pqueue2, &aux);
      sizeque++;
    }
    ...
    {
      QueueDequeue(pqueue2,&aux);
      Array[i]=aux;
    }

    for(int i=0; i< sizeque;i++)
    {
      for(int j=i; j< sizeque;j++)
      {
          if(Array[j]<Array[i]){
            aux2=Array[i];
            Array[i]=Array[j];
...
int ABPIsEvenOdd (PtABPNode proot)
{
  if(proot == NULL){Error = ABP_EMPTY; return 0;}
  Error = OK;
	PtABPNode Temp = ABPCopy(proot);
  int a;
  a=ABPMinRec(Temp)->Elem ;
  ABPDeleteMin (&Temp);
	while (Temp != NULL) {
    if((a%2==0 && (ABPMinRec(Temp)->Elem)%2==0) ||  (a%2!=0 && (ABPMinRec(Temp)->Elem)%2!=0)) {return 0;}
     a=ABPMinRec(Temp)->Elem;
		ABPDeleteMin (&Temp);
	}
  return 1;

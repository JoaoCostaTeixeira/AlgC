...
{
		PtSeqList pseq;
		if((pseq = malloc (sizeof (struct seqlist))) == NULL){Error = NO_MEM; return NULL;}
...
// Não Esta funcional apenas esta a adicionar o primeiro, depois nao adiciona mais nenhum dando
// Erro
void SeqListInsert (PtSeqList pseq, int pvalue)
{
	 if(( pseq==NULL)){ Error = NO_SEQ; return ; }
	 PtBiNode Node;
	 if((Node = BiNodeCreate(pvalue)) == NULL){Error = NO_MEM; return ;}

		Error = OK;
		if( pseq -> Size == 0){
			pseq -> Head = Node;
			pseq -> Tail = Node;
			pseq -> Tail -> PtPrev = pseq -> Head;
			pseq -> Head -> PtNext = pseq -> Tail;
			pseq -> Size =1;
		}
		else if( pseq -> Size == 1){
				Node -> PtPrev = pseq -> Head;
				pseq -> Head -> PtNext = Node;
				pseq -> Tail = Node;
				pseq -> Size ++;
		}else{
			PtBiNode Node2 = pseq -> Tail;
			Node2 -> PtNext = Node;
			Node -> PtPrev = Node2;
			pseq -> Tail = Node;
			pseq -> Tail -> PtPrev -> PtPrev -> PtNext = Node2;
			pseq -> Size ++;}
}
...
void SeqListDelete (PtSeqList pseq, int pvalue)
{
	...
	int aux = 0; // auxiliar para caso encontre ou nao o valor
	PtBiNode Node;
	if(pseq -> Head -> Elem == pvalue){
		aux=1;
		Node = pseq ->Head;
	...
	if(pseq -> Tail -> Elem == pvalue){
		 aux=1;
		 Node = pseq ->Tail;
		 pseq->Tail = pseq -> Tail -> PtPrev;
		 BiNodeDestroy(&Node);
		 pseq -> Size --;}
  Node = pseq -> Head -> PtNext;
...
int SeqListIsOrdered (PtSeqList pseq)
{
  ...
  {
    if(Node -> Elem < Node -> PtPrev -> Elem ){return 0;}
		Node = Node -> PtNext;
  }
  return 1;
}
...
int SeqListIsEvenOdd (PtSeqList pseq)
{
	...
	{
			if(((Node -> Elem%2==0) && (Node -> PtPrev -> Elem%2==0))
			 || ((Node -> Elem%2!=0) && (Node -> PtPrev -> Elem))){return 0;}
			 Node = Node -> PtNext;
...
void SeqListSmallerBigger (PtSeqList pseq, int * psmall, int * pbig)
{
	...
	for(int i=0; i<pseq->Size;i++)
	{
			if((Node -> Elem< min )){min = Node -> Elem;}
			if((Node -> Elem> max )){max = Node -> Elem;}
			Node = Node -> PtNext;
	}
	*psmall = min;
	*pbig = max;
...
double SeqListAverage (PtSeqList pseq)
{
	...
	PtBiNode Node;
	double sum=0;
	for(Node = pseq -> Head; Node!=NULL;Node=Node->PtNext){sum = sum + Node -> Elem;}
